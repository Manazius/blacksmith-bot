...
def dumpM(u):
    ...
    print 'id=%d,' % id,
    print 'qr=%(qr)s, opcode=%(opcode)s, aa=%(aa)s, tc=%(tc)s, rd=%(rd)s, ra=%(ra)s, z=%(z)s, rcode=%(rcode)s,' % vars()
    if tc:
        print '*** response truncated! ***'
    if rcode:
        print '*** nonzero error code! (%d) ***' % rcode
    print '  qdcount=%(qdcount)s, ancount=%(ancount)s, nscount=%(nscount)s, arcount=%(arcount)s' % vars()
...
class DnsResult:
    ...
    def show(self):
        ...
        print ';; flags: %s; Ques: %d, Ans: %d, Auth: %d, Addit: %d' % (string.join(flags),
...
#        ks = Class.classstr(klass)
#        print 'name=%(name)s, type=%(type)s(%(typename)s), class=%(klass)s(%(ks)s), ttl=%(ttl)s' % vars()
...
def dumpQ(u):
    ...
    print 'qname=%s, qtype=%d(%s), qclass=%d(%s)' % (qname,
        qtype, Type.typestr(qtype), qclass, Class.classstr(qclass))
